...
class ProfileContainer extends React.Component<PropsProFileContainerType> {
    refreshProfile() {
        ...
        if (!userId) {
            const MyUserId = this.props.authorizedUserId;
            if (!MyUserId) {
        ...
        this.props.getUserProfile(parseInt(userId));
...
export default compose(
    connect<MapStateToPropsType, DispatchToPropsType, unknown, AppStateType>(mapStateToProps, {
        getUserProfile,
        getStatus,
        savePhoto,
        updateStatus,
